...
CloudWatch Logsで確認：
- Lambda で `MemoApi` を選択 → モニタリング タブを選択 → CloudWatch ログを表示
...
JWT（**JSON Web Token**）は、**「ユーザーが誰か／どんな権限があるか」などの情報を、改ざんできない形で持ち運ぶためのトークン**です。ログイン後にクライアント（ブラウザやアプリ）が受け取り、API 呼び出し時に一緒に送ります。

#### JWTの特徴（何が嬉しい？）
- **自己完結**：トークンの中に必要な情報（ユーザーID、期限など）が入っている
- **署名付き**：発行者（例: Cognito）が署名するので、受け取る側（API Gatewayなど）は**改ざんされていない**ことを検証できる
- **期限付き**：`exp`（有効期限）で「いつまで有効か」を判断できる

※「暗号化」ではなく、基本は**署名**です。中身は Base64URL なので、**見ようと思えば読めます**（ただし改ざんは検証で弾かれる）。

#### JWTの形（3つのパート）
JWTはだいたいこんな形で、`.` で3つに分かれます：

```plain text
header.payload.signature
```

- **header**：署名方式など（例: RS256）
- **payload**：クレーム（主張）＝中身のデータ（例: `iss`, `aud`, `sub`, `exp`）
- **signature**：header+payloadに対する署名（改ざん検知用）

#### よく出るクレーム（項目）
- `iss`（issuer）: 発行者（誰が出したトークンか）
- `aud`（audience）: 想定受取人（どのアプリ向けか）
- `sub`（subject）: ユーザー識別子
- `exp` : 有効期限（この時刻以降は無効）
- `iat` : 発行時刻

#### 今回の構成（Cognito + API Gateway）だと
- Cognito が JWT を発行
- API Gateway の JWT Authorizer が
    - `iss`（issuer）が Cognito のUser Poolのものか
    - `aud` が指定したクライアントID か
    - 署名が正しいか、期限切れでないか  
      をチェックして、OKならリクエストを通します。
...
public class MemoApiStack extends Stack {
  ...
  public MemoApiStack(final Construct scope, final String id, final StackProps props) {
    ...
    HttpJwtAuthorizer authorizer = HttpJwtAuthorizer.Builder.create("JwtAuthorizer", issuer)
...
`issuer（発行者）` は、**その JWT（トークン）を「誰が発行したか」を表す識別子**です。JWT のクレーム（中身）では通常 **`iss`** という項目に入り、検証側（今回だと API Gateway の JWT Authorizer）が「このトークンは信頼できる発行元から来たものか？」を判断するために使います。

#### 今の Cognito の例での issuer
Cognito User Pool が発行するトークンの issuer は、だいたい次の形式です。
...
#### 何のために必要？
API Gateway の JWT Authorizer は主に次を確認します：

1. **署名が正しいか**（Cognito が公開している公開鍵で検証できるか）
2. **`iss` が期待した issuer と一致するか**（別のUser Poolや別リージョンのトークンを弾く）
3. **`aud`（Audience）が期待値に合うか**（`jwtAudience` で指定したクライアントIDなど）

つまり issuer は、**「このトークンはこの Cognito User Pool が発行したものだ」と確定するための根拠**です。

#### ありがちな落とし穴
- **リージョンや UserPoolId が違う issuer**を入れると、正しいトークンでも認証で落ちます。
- `issuer` は「ログイン画面のURL」みたいなものではなく、**トークンの `iss` と一致する“発行元識別子”** です。
...
```shell script
cdk bootstrap
# 認証でエラーが発生した場合は、アカウントIDとリージョンを明示する
# 例：
